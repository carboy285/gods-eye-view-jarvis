...
export const CREDENTIALS = Object.freeze([
  ...
  { name: 'NVIDIA_API_KEY', label: 'NVIDIA NIM voice agent', keychain: [] },
  { name: 'META_MODEL_API_KEY', label: 'Meta Muse voice agent', keychain: [] },
  { name: 'COMPOSIO_API_KEY', label: 'Composio app tools', keychain: [] },
  { name: 'NTFY_TOPIC', label: 'Phone push (ntfy topic)', keychain: [] },
  { name: 'INKBOX_API_KEY', label: 'iMessage (Inkbox)', keychain: [] },
  { name: 'INKBOX_OWNER_NUMBER', label: 'iMessage owner number', keychain: [] },
  {
...
export function resolveOpenSkyAuthMode({
  ...
  const environmentDefinesMode = Object.prototype.hasOwnProperty.call(
    environment,
    'OPENSKY_AUTH_MODE',
  );
  const environmentMode = String(environment.OPENSKY_AUTH_MODE || '').trim();
  const dotenvMode =
    authoritativeEnvironment && environmentDefinesMode
      ? ''
...
export function buildCapabilitySummary(
  ...
  const requestedOpenSkyMode = String(openSkyAuthMode || '')
    .trim()
    .toLowerCase();
  ...
  return {
    ...
    voice: `${
      configured('NVIDIA_API_KEY')
        ? 'NVIDIA NIM agent with NVIDIA Magpie voice'
        : configured('META_MODEL_API_KEY')
          ? 'Meta Muse agent'
          : configured('OPENAI_API_KEY')
            ? 'available'
            : 'off until an OpenAI, NVIDIA or Meta Muse key is added'
    }${
      configured('COMPOSIO_API_KEY') &&
      (configured('NVIDIA_API_KEY') || configured('META_MODEL_API_KEY'))
        ? ', plus your apps via Composio'
        : ''
    }`,
...
export function inspectSetup({
  ...
  return {
    ...
    capabilities: buildCapabilitySummary(credentials, {
      openSkyAuthMode: resolveOpenSkyAuthMode({
        authoritativeEnvironment,
        rootDir,
      }),
    }),
