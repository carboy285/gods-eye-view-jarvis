 ...
 *
 * Loopback and GEV_TRUSTED_IPS only: with HOST=0.0.0.0 the app can be shared
 * on a LAN, and a guest must be able to neither write the host's .env nor
 * probe which keys exist; only the owner's listed devices get this machine's
 * access. Prod builds never register this middleware (apply: 'serve'), so
 * the panel's status fetch fails and the client removes the whole surface.
 */
/**
 * The authority the browser addressed. HTTP/2 (every browser over HTTPS)
 * sends `:authority` instead of a Host header; both get the same gate.
 */
function requestAuthority(req) {
  return req.headers?.host || req.headers?.[':authority'] || undefined;
}
...
function keySetupEndpoint({ sourceRoot = defaultSourceRoot } = {}) {
  ...
  const admit = (req) =>
    admitKeySetupRequest({
      ...
      hostHeader: requestAuthority(req),
...
export { keySetupEndpoint, requestAuthority };
