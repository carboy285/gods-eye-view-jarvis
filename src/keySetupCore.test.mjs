...
test('external ownership uses boot provenance even when store and shell values match', () => {
  assert.equal(
    ...
    true,
    'equal bytes cannot turn a shell/Keychain value into a file-owned value',
  );
  assert.equal(
    isKeySetupExternallyManaged({
    ...
    false,
    'a value loaded only from the owned store remains editable',
  );
  assert.equal(
    isKeySetupExternallyManaged({
    ...
    true,
    'a differing live value remains external',
  );
  assert.equal(
    isKeySetupExternallyManaged({
      effectiveValue: '',
    ...
    false,
    'an absent live credential has no external owner',
  );
});
...
test('subprocess success requires a clean zero exit', () => {
  ...
  assert.equal(
    commandCompletedSuccessfully({ status: 1, signal: null }),
    false,
  );
  assert.equal(
    commandCompletedSuccessfully({ status: 0, signal: 'SIGTERM' }),
    false,
  );
  assert.equal(
    commandCompletedSuccessfully({
...
test('Windows owner SID parsing reads only the structured user-SID CSV field', () => {
  ...
  assert.equal(
    parseWindowsUserSid(`"WORKSTATION\\alice","${localUser}"\r\n`),
    localUser,
  );
  assert.equal(
    parseWindowsUserSid(`"AzureAD\\alice","${entraUser}"`),
    entraUser,
  );
  assert.equal(
  ...
  assert.equal(
    parseWindowsUserSid('"WORKSTATION\\alice","S-1-5-32-545"'),
    null,
    'broad group SID refused',
  );
  assert.equal(
    parseWindowsUserSid(
      `"WORKSTATION\\alice","${localUser}"\n"extra","${localUser}"`,
    ),
    null,
  );
  assert.equal(
...
test('validation accepts every registry env var and only those', () => {
  ...
  assert.equal(
    validateKeySetupUpdates({ PATH: '/usr/bin' }).ok,
    false,
    'PATH must be refused',
  );
  assert.equal(
    validateKeySetupUpdates({ NODE_OPTIONS: '--x' }).ok,
    false,
    'NODE_OPTIONS must be refused',
  );
});
...
test('validation trims, and refuses empties, newlines, spaces, and oversize values', () => {
  ...
  assert.equal(validateKeySetupUpdates({ OPENAI_API_KEY: '   ' }).ok, false);
  assert.equal(
    validateKeySetupUpdates({ OPENAI_API_KEY: 'a\nb' }).ok,
    false,
    'newline injection',
  );
  assert.equal(
    validateKeySetupUpdates({ OPENAI_API_KEY: 'a b' }).ok,
    false,
    'inner space',
  );
  assert.equal(
    validateKeySetupUpdates({ OPENAI_API_KEY: 'kéy' }).ok,
    false,
    'non-ASCII',
  );
  assert.equal(
...
test('upsert appends unknown keys under one shared header, once', () => {
  ...
  assert.equal(
    first,
    ['PORT=4173', '', KEY_SETUP_APPEND_HEADER, 'OPENAI_API_KEY=sk-1'].join(
      '\n',
    ) + '\n',
  );
  ...
  assert.equal(
    second.split(KEY_SETUP_APPEND_HEADER).length,
    2,
    'header written once',
  );
});
...
test('a real .env.example round-trip: the curated file keeps its shape', () => {
  ...
  assert.equal(
    lines[7],
    'TOMTOM_API_KEY=tt-real',
    'commented key uncommented in place',
  );
});
...
test('the admission gate refuses every non-local shape, one assertion per refusal', async () => {
  ...
  // A trusted device (GEV_TRUSTED_IPS) reaches the server by its LAN IP.
  const mac = {
    ...local,
    remoteAddress: '::ffff:192.168.1.50',
    hostHeader: '192.168.1.10:4173',
    origin: 'http://192.168.1.10:4173',
    env: { GEV_TRUSTED_IPS: '192.168.1.50' },
  };
  assert.equal(
    admitKeySetupRequest(mac).ok,
    true,
    'a trusted device is admitted by IP',
  );
  assert.equal(
    admitKeySetupRequest({ ...mac, remoteAddress: '192.168.1.90' }).ok,
    false,
    'an untrusted neighbour is refused',
  );
  assert.equal(
    admitKeySetupRequest({ ...mac, env: {} }).ok,
    false,
    'no trusted list means loopback only',
  );
  assert.equal(
    admitKeySetupRequest({ ...mac, hostHeader: 'rebind.evil:4173' }).ok,
    false,
    'DNS rebinding carries a name, still refused',
  );
  assert.equal(
    admitKeySetupRequest({ ...mac, origin: 'https://evil.example' }).ok,
    false,
    'a hostile page on the trusted device is refused',
  );
  assert.equal(
    admitKeySetupRequest({
      ...mac,
      env: { ...mac.env, PINOKIO_SHARE_LOCAL: '1' },
    }).ok,
    false,
    'sharing still disables the surface',
  );
  assert.equal(
    admitKeySetupRequest({
      ...local,
    ...
    false,
    'loopback keeps the strict localhost Host rule',
  );
...
test('a null value validates as a removal; an empty string still does not', () => {
  ...
  assert.equal(
    validateKeySetupUpdates({ OPENAI_API_KEY: '' }).ok,
    false,
    'empty is a mistake, not a removal',
  );
  assert.equal(
    validateKeySetupUpdates({ PATH: null }).ok,
    false,
    'removal is registry-bound too',
  );
});
...
test('removal comments the assignment back out, returning the file to template shape', () => {
  ...
  assert.equal(
    lines[1],
    '# OPENAI_API_KEY=',
    'active line commented out, not deleted',
  );
...
test('validation rejects dotenv metacharacters that would round-trip wrong', () => {
  for (const bad of ['abc#def', 'ab"cd', "ab'cd", 'ab$cd', 'ab\\cd', 'ab`cd']) {
    assert.equal(
      validateKeySetupUpdates({ OPENAI_API_KEY: bad }).ok,
      false,
  ...
  for (const good of [
    'sk-AbC0-9_x',
    'eyJhbGc.eyJzdWI.QWxpY2U',
    'a1b2c3d4e5f6',
    'AB+cd/ef=',
  ]) {
    assert.equal(
      validateKeySetupUpdates({ OPENAI_API_KEY: good }).ok,
      true,
      `${good} accepted`,
    );
  }
...
test('server Google key remains supported without appearing in setup or its missing count', () => {
  ...
  assert.deepEqual(
    validateKeySetupUpdates({ GOOGLE_MAPS_SERVER_API_KEY: secret }),
    {
      ok: true,
      updates: { GOOGLE_MAPS_SERVER_API_KEY: secret },
    },
  );
  assert.equal(
    validateKeySetupUpdates({ GOOGLE_MAPS_SERVER_API_KEY: null }).ok,
    true,
  );
  ...
  assert.equal(
    status.keys.find((key) => key.id === 'google-maps').title,
    'GOOGLE MAPS',
  );
  const allVisibleConfigured = Object.fromEntries(
    status.keys.flatMap((key) =>
  ...
  assert.equal(
    complete.setCount,
    complete.total,
    'an absent server key must not leave setup incomplete',
  );
  assert.deepEqual(
    complete,
    keySetupStatus({
