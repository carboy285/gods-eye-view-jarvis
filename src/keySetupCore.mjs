...
export const KEY_SETUP_KEYS = Object.freeze([
  ...
  Object.freeze({
    id: 'nvidia',
    title: 'NVIDIA NIM',
    unlocks: 'Voice agent with browser speech — replaces OpenAI voice',
  ...
  Object.freeze({
    id: 'composio',
    title: 'COMPOSIO',
    unlocks: 'Voice agent reaches your apps: email, calendar, chat, code',
    getUrl: 'https://dashboard.composio.dev',
    envVars: Object.freeze(['COMPOSIO_API_KEY']),
    tier: 'free',
  }),
  Object.freeze({
    id: 'ntfy',
    title: 'PHONE (NTFY)',
    unlocks: 'Jarvis reminders and alerts pushed to your phone',
    getUrl: 'https://ntfy.sh/app',
    envVars: Object.freeze(['NTFY_TOPIC']),
    tier: 'free',
  }),
  Object.freeze({
    id: 'inkbox',
    title: 'IMESSAGE (INKBOX)',
    unlocks: 'Text Jarvis over iMessage; only your own number is answered',
    getUrl: 'https://inkbox.ai/console/api-keys',
    envVars: Object.freeze(['INKBOX_API_KEY', 'INKBOX_OWNER_NUMBER']),
    tier: 'free',
  }),
  Object.freeze({
    id: 'muse',
    title: 'META MUSE',
    unlocks: 'Muse voice agent — used when no NVIDIA key is set',
    getUrl: 'https://dev.meta.ai',
    envVars: Object.freeze(['META_MODEL_API_KEY']),
    tier: 'metered',
  }),
  Object.freeze({
    id: 'aisstream',
...
function isLoopbackAddress(remoteAddress) {
  return LOOPBACK_ADDRESSES.has(normalizeAddress(remoteAddress));
}

/**
 * This machine, plus the devices listed in GEV_TRUSTED_IPS, which get the
 * same access as this machine (Provider Settings and the agent's app tools).
 * Judged by the socket peer only; forwarded headers never count.
 */
export function isTrustedClientAddress(remoteAddress, env = {}) {
  const address = normalizeAddress(remoteAddress);
  if (!address) return false;
  if (LOOPBACK_ADDRESSES.has(address)) return true;
  return String(env.GEV_TRUSTED_IPS || '')
    .split(',')
...
/**
 * Parse an exact local request authority from a Host header. A trusted LAN
 * device reaches this machine by IP address, so it may also use an IP-literal
 * Host; DNS rebinding always carries a domain name, which stays refused.
 */
function localAuthority(hostHeader, protocol, { allowIpLiteral = false } = {}) {
  ...
  try {
    ...
    return LOCAL_HOSTNAMES.has(hostname) ||
      (allowIpLiteral && isIpLiteralHostname(hostname))
      ? parsed.origin
      : null;
  } catch {
    return null;
...
export function admitKeySetupRequest({
  ...
  if (!isTrustedClientAddress(remoteAddress, env)) {
    return {
      ok: false,
      status: 403,
      error:
        'Provider Settings answers only this machine and devices in GEV_TRUSTED_IPS',
    };
  }
  const authority = localAuthority(hostHeader, protocol, {
    allowIpLiteral: !isLoopbackAddress(remoteAddress),
  });
  if (!authority) {
    return {
      ok: false,
      status: 403,
      error: 'Provider Settings answers only local hostnames and IP addresses',
    };
  }
  if (
    method === 'POST' &&
