...
test('doctor describes the credential ladder without exposing values', () => {
  const credentials = {
    ...
    META_MODEL_API_KEY: { configured: false },
    COMPOSIO_API_KEY: { configured: false },
    NTFY_TOPIC: { configured: false },
    INKBOX_API_KEY: { configured: false },
    INKBOX_OWNER_NUMBER: { configured: false },
  ...
  assert.equal(
    buildCapabilitySummary({
      ...credentials,
      META_MODEL_API_KEY: { configured: true, source: 'dotenv files' },
    }).voice,
    'Meta Muse agent',
  );
  assert.equal(
    buildCapabilitySummary({
      ...credentials,
      NVIDIA_API_KEY: { configured: true, source: 'dotenv files' },
      META_MODEL_API_KEY: { configured: true, source: 'dotenv files' },
    }).voice,
  ...
  assert.equal(
    buildCapabilitySummary({
      ...credentials,
      NVIDIA_API_KEY: { configured: true, source: 'dotenv files' },
      COMPOSIO_API_KEY: { configured: true, source: 'dotenv files' },
    }).voice,
    'NVIDIA NIM agent with NVIDIA Magpie voice, plus your apps via Composio',
  );
  assert.equal(
    buildCapabilitySummary({
      ...credentials,
    ...
    'available',
    'Composio needs an agent to use it',
  );
